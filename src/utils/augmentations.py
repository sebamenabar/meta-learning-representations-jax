...
augment_config = dict(
    ...
    view2=dict(
        ...
        gaussian_blur=dict(
            apply_prob=0.1, blur_divider=10.0, sigma_min=0.1, sigma_max=2.0
        ),
...
def postprocess(inputs: JaxBatch, rng: jnp.ndarray):
    ...
    def _postprocess_image(
        images: jnp.ndarray, rng: jnp.ndarray, presets: ConfigDict,
    ) -> JaxBatch:
...
def gaussian_blur(
    images, rng, blur_divider=10.0, sigma_min=0.1, sigma_max=2.0, apply_prob=1.0
):
...
def _random_crop_single_image(img, rng, out=84, padding=None):
    """
        args:
        imgs: np.array shape (H,W,C)
        out: output size (e.g. 84)
        returns np.array
    """
    rng_w, rng_h = jax.random.split(rng)
    if padding:
        img = jnp.pad(img, padding)
    h, w, c = img.shape
    crop_max = h - out + 1
    w1 = jax.random.randint(rng_w, [], 0, crop_max)
    h1 = jax.random.randint(rng_h, [], 0, crop_max)
...
def random_crop(images, rng, out, padding=None):
    rngs = jax.random.split(rng, images.shape[0])
    return jax.vmap(
        functools.partial(_random_crop_single_image, out=out, padding=padding)
    )(images, rngs)
